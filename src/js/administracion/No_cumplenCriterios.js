...
import { observableAxios__informacionBeneficiario__no, observableAxios__informacionBeneficiario__excel__no__cumplen } from '@/js/axios/index.js';
...
export default {
  ...
  methods: {
    ...
    cargarData(d, callback) {
      forkJoin([observableAxios__informacionBeneficiario__no(d)]).subscribe({
        next: (results) => {
    ...
    excelReporteriaTotal() {
      ...
      forkJoin([observableAxios__informacionBeneficiario__excel__no__cumplen(params)])
        .subscribe({
          next: (results) => {
    ...
    exportarTodoFiltrado() {
      ...
      forkJoin([observableAxios__informacionBeneficiario__excel__no__cumplen(params)]).subscribe({
        next: (results) => {
          const res = results[0];
          const info = res.data || [];
